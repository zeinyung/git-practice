...
| `curl -v http://www.baidu.com` | 显示请求头+响应头完整对话（`>` 发出的 `<` 收到的） | 排查 HTTP 层问题 |
| `curl -I http://www.baidu.com` | 只发 HEAD 请求，不看内容 | 检查服务是否活着，看一眼 200 OK 就行 |
| `curl -s -o /dev/null -w "%{http_code}\n" URL` | 只输出状态码 | 脚本里判断服务健康 |
| `curl -X POST URL -H "Content-Type: application/json" -d '{"key":"val"}'` | 发 POST 请求带 JSON 数据 | 调 API、测试接口 |
| `curl -O URL` | 下载文件 | 下载安装包 |
...
```bash
user="张三"
echo ${user}              # $变量名 = 取出变量存的文字

echo $(date)              # $(命令) = 执行命令，把命令输出变成文字
```
